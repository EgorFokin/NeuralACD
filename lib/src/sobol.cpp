...
#define LOG_MAX 62
  ...
  //  in some cases, a compiler was complaining that the value of ATMOST could
  //  not seem to be properly stored.  We only need ATMOST in order to specify
  //  MAXCOL, so as long as we set MAXCOL (below) to what we expect it should
  //  be, we may be able to get around this difficulty. JVB, 24 January 2006.
  //
  ...
  if (!initialized || dim_num != dim_num_save) {
    ...
    //  order to specify MAXCOL, so if we know what the answer should be we can
    //  try to get it this way! JVB, 24 January 2006.
    //
