...
class ACDgen(IterableDataset):
    ...
    def __iter__(self):
        ...
        while True:
            ...
            structure_type = np.random.choice(self.config.generation.struct_types)
            ...
            values = self.get_distances(points, structure.cut_verts)
            ...
            values = 1 - (torch.clamp(values, 0.01, 0.03)-0.01)*50
            # distances[distances != 0] = 1

            if self.output_meshes:
                yield points, values, structure
            else:
                yield points, values